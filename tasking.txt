indoor localization with bluetooth or inertial sensor

Goal:
	object localization based on motion sensor infomation, bluetooth information and even visual information
...
Current problem:
	...
	The paper mainly focuse on device in the arms to tracking movement. But this is not helpful with our task. Our task is to find the distance and angle with two device. But the movement tracking only provide us with 
But we need a server and map in this environment, and we need to set the original position of our device manually. The scenario we want to invesitigate seems like don't need a map. So I begin to get confused.
